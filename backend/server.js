...
const AppContent = () => {
  ...
  const [priority, setPriority] = useState('Medium'); // Now fully used
  ...
  const welcomeData = {
    ...
    quote: quotes[new Date().getHours() % quotes.length]
  };
  ...
  const loadTasks = useCallback(async (token) => {
    try {
      ...
      setTasks(res.data);
    } catch (err) { console.error("Session sync paused"); }
  }, []);
  ...
  const syncProfile = async (n, t, a) => {
    ...
    } catch (err) { console.log("Local-only mode active"); }
  };
  ...
  const handleAuth = async (e) => {
    ...
    try {
      ...
      navigate('/');
    } catch (err) { alert("Auth Error: Check Railway Database connection."); }
  };
  ...
  const addTask = async (e) => {
    ...
    alert("TaskPro: Node Deployed & Email Notification Dispatched!");
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#020617] transition-all duration-500 font-sans text-slate-900 dark:text-slate-100">
      ...
      <AnimatePresence>
        {showProfile && user && (
          <motion.div initial={{ opacity: 0, x: 300 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 300 }} className="fixed right-0 top-0 h-full w-80 glass p-8 z-[100] border-l dark:border-white/10 shadow-2xl">
            <div className="flex justify-between items-center mb-10"><h4 className="font-black text-xs uppercase tracking-widest">Workspace Settings</h4><X size={24} className="cursor-pointer hover:text-red-500" onClick={() => setShowProfile(false)} /></div>
            <div className="space-y-8">
              <div className="space-y-4">
                ...
                <div className="flex gap-3 bg-slate-100 dark:bg-slate-900 p-4 rounded-3xl">
                  {isEditingName ? (
                    <><input className="bg-transparent border-b border-indigo-500 outline-none w-full font-bold dark:text-white" value={newName} onChange={e => setNewName(e.target.value)} /><Check size={20} className="text-green-500 cursor-pointer" onClick={() => { setUser({...user, name: newName}); setIsEditingName(false); syncProfile(newName, themeColor, selectedAvatar); }} /></>
                  ) : (
                    <><span className="font-bold flex-1 dark:text-white">{user.name}</span><Edit3 size={16} className="text-indigo-500 cursor-pointer" onClick={() => {setNewName(user.name); setIsEditingName(true);}} /></>
                  )}
                </div>
              </div>
              <div className="space-y-4">
                <label className="text-[10px] font-black uppercase text-slate-500">Avatar Selection</label>
                <button onClick={() => setShowAvatarPicker(!showAvatarPicker)} className="w-full p-2 bg-indigo-500/10 text-indigo-500 rounded-xl text-[10px] font-bold uppercase tracking-widest"><Star size={12} className="inline mr-1" /> {showAvatarPicker ? "Close Gallery" : "Open Gallery"}</button>
                {showAvatarPicker && (
                  <div className="grid grid-cols-4 gap-2 animate-in zoom-in">
                    {avatars.map(a => <button key={a} onClick={() => {setSelectedAvatar(a); syncProfile(user.name, themeColor, a);}} className={`p-1 rounded-xl transition-all ${selectedAvatar === a ? 'bg-indigo-500 ring-2 ring-white' : 'bg-slate-200 dark:bg-slate-800'}`}><img src={`https://api.dicebear.com/7.x/pixel-art/svg?seed=${a}`} alt="a"/></button>)}
                  </div>
                )}
              </div>
              <div className="space-y-4">
              ...
              <button onClick={() => {localStorage.clear(); window.location.href="/";}} className="w-full mt-10 p-5 rounded-3xl bg-red-500 text-white font-black uppercase text-[10px] tracking-widest shadow-xl shadow-red-500/20 flex items-center justify-center gap-2 transition-all hover:bg-red-600"><LogOut size={16}/> Sign Out</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <Routes>
        {/* --- ABOUT PAGE (DEVELOPED BY YESASWI) --- */}
        <Route path="/about" element={
          <div className="max-w-4xl mx-auto py-20 px-6 text-center">
            <h1 className="text-7xl font-black mb-6">Developed by <span className={themes[themeColor].split(' ')[0]}>Yesaswi</span></h1>
            <p className="text-slate-500 text-xl mb-16 font-medium italic">"Precision is the difference between a student and a professional."</p>
            <div className="grid md:grid-cols-3 gap-8">
              <div className="glass p-10 rounded-[3rem] border-t-[12px] border-indigo-600 shadow-2xl text-center"><Shield className="mx-auto text-indigo-500 mb-4" size={40}/><h4 className="font-black mb-2 uppercase">Secure JWT</h4><p className="text-sm text-slate-400">Industry-standard authentication layer.</p></div>
              <div className="glass p-10 rounded-[3rem] border-t-[12px] border-emerald-600 shadow-2xl text-center"><Rocket className="mx-auto text-emerald-500 mb-4" size={40}/><h4 className="font-black mb-2 uppercase">Native Flow</h4><p className="text-sm text-slate-400">Optimized MongoDB native driver engine.</p></div>
              <div className="glass p-10 rounded-[3rem] border-t-[12px] border-amber-600 shadow-2xl text-center"><Cpu className="mx-auto text-amber-500 mb-4" size={40}/><h4 className="font-black mb-2 uppercase">Automation</h4><p className="text-sm text-slate-400">Non-blocking background notification system.</p><div className="mt-4"><Code size={14} className="mx-auto opacity-20"/></div></div>
            </div>
          </div>
        }/>
        ...
        <Route path="/contact" element={
          <div className="max-w-xl mx-auto py-20 px-6 text-center">
            ...
            <h2 className="text-5xl font-black uppercase tracking-tighter">Support</h2>
            <p className="font-bold text-slate-500 mt-4 mb-10">Email: myselfadmin123@gmail.com</p>
            <form className="glass p-10 rounded-[3.5rem] space-y-4 shadow-3xl border dark:border-white/5" onSubmit={(e)=>{e.preventDefault(); alert("Inquiry Dispatched!")}}>
               <input className="w-full p-5 glass rounded-3xl outline-none" placeholder="Student Context" required />
               <textarea className="w-full p-5 glass rounded-3xl outline-none h-40" placeholder="Technical details..." required />
          ...
          <div className="min-h-[85vh] flex items-center justify-center p-6 bg-[url('https://images.unsplash.com/photo-1497215728101-856f4ea42174?auto=format&fit=crop&q=80&w=2000')] bg-cover relative">
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="glass p-12 rounded-[3.5rem] w-full max-w-md shadow-2xl border dark:border-white/5 text-center relative z-10">
              ...
              <form onSubmit={handleAuth} className="space-y-4 text-left">
                {isRegister && <input className="w-full p-4 bg-gray-100/10 dark:bg-slate-900 rounded-2xl outline-none dark:text-white border border-white/10" placeholder="Full Student Name" onChange={e => setAuthForm({...authForm, name: e.target.value})} required />}
        ...
        <Route path="/" element={!user ? (
          <div className="min-h-[85vh] flex flex-col items-center justify-center text-center px-6 relative overflow-hidden">
            ...
            <div className="bg-indigo-500/10 p-8 rounded-[2.5rem] border border-indigo-500/20 mb-12 flex items-center gap-4 max-w-2xl shadow-inner">
               <Quote className="text-indigo-500" size={32}/><p className="text-2xl italic font-medium">"{welcomeData.quote}"</p>
            </div>
            <Link to="/login" className="bg-indigo-600 text-white px-14 py-7 rounded-[2.5rem] font-black text-2xl shadow-3xl hover:scale-110 transition-all border border-white/10 shadow-indigo-500/20">Launch Workspace</Link>
          </div>
        ) : (
          <main className="max-w-7xl mx-auto p-4 md:p-12 flex flex-col lg:flex-row gap-12">
            <div className="w-full lg:w-1/3 flex flex-col gap-8">
              <div className={`p-10 rounded-[3.5rem] text-white shadow-2xl relative overflow-hidden flex flex-col justify-between min-h-[300px] ${themes[themeColor].split(' ')[1]}`}>
                ...
                <div><div className="w-full bg-white/20 h-3 rounded-full overflow-hidden mb-6 shadow-inner"><motion.div animate={{ width: `${progress}%` }} className="bg-white h-full shadow-[0_0_20px_white]" /></div><p className="text-[10px] font-black uppercase tracking-widest flex items-center gap-2"><Trophy size={14} /> {Math.round(progress)}% Mastery</p></div>
              ...
              <form onSubmit={addTask} className="glass p-8 rounded-[3.5rem] space-y-4 border dark:border-white/5 shadow-2xl">
                ...
                <input className="w-full p-5 bg-gray-50 dark:bg-slate-900 rounded-3xl outline-none dark:text-white border-2 border-transparent focus:border-indigo-500 transition-all shadow-inner" placeholder="Project Name" value={taskTitle} onChange={e => setTaskTitle(e.target.value)} required />
                <textarea className="w-full p-5 bg-gray-50 dark:bg-slate-900 rounded-3xl outline-none dark:text-white h-24 border-2 border-transparent focus:border-indigo-500 transition-all shadow-inner" placeholder="Description..." value={taskDesc} onChange={e => setTaskDesc(e.target.value)} />
                ...
                <div className="flex gap-2">
                   {['Low', 'Medium', 'High'].map(p => (
                     <button key={p} type="button" onClick={() => setPriority(p)} className={`flex-1 py-2 rounded-xl text-[9px] font-black uppercase transition-all ${priority === p ? themes[themeColor].split(' ')[1] + ' text-white shadow-xl scale-105' : 'bg-gray-100 dark:bg-slate-800 text-slate-500'}`}>{p}</button>
                   ))}
                </div>
                <button className={`w-full ${themes[themeColor].split(' ')[1]} text-white p-5 rounded-[2.5rem] font-black text-lg shadow-xl shadow-current/30 mt-4 active:scale-95 transition-all`}>Deploy Task</button>
              </form>
            </div>

            <div className="lg:col-span-8 space-y-8">
              <div className="flex flex-col md:flex-row gap-6 justify-between items-center bg-white dark:bg-slate-900 p-8 rounded-[3rem] shadow-xl border dark:border-white/5 relative overflow-hidden">
                <div className="relative w-full md:w-[24rem]"><Search className="absolute left-6 top-5 text-slate-400" size={20} /><input placeholder="Search projects..." className="w-full pl-16 p-5 glass rounded-full outline-none dark:text-white border-2 border-transparent focus:border-indigo-500 shadow-inner" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} /></div>
                <div className="flex bg-gray-100 dark:bg-slate-800 px-8 py-4 rounded-full border dark:border-white/5 gap-6 items-center shadow-inner">
                   <div className="flex items-center gap-3"><Bell size={20} className={progress === 100 ? "text-green-500" : "text-gray-500"} /><Timer size={24} className={isTimerRunning ? "animate-pulse text-indigo-500" : "text-gray-500"}/><span className="font-mono font-black text-2xl leading-none dark:text-white">{Math.floor(timeLeft/60)}:{timeLeft%60 < 10 ? '0' : ''}{timeLeft%60}</span></div>
                   <button onClick={() => setIsTimerRunning(!isTimerRunning)} className={`text-[10px] font-black uppercase py-2 px-4 rounded-xl ${themes[themeColor].split(' ')[1]} text-white shadow-xl`}>{isTimerRunning ? "Stop" : "Focus"}</button>
                </div>
              </div>
              <div className="flex bg-gray-200 dark:bg-slate-900 p-2 rounded-3xl w-max shadow-inner">
                  {['All', 'To-Do', 'Completed'].map(f => (
                    <button key={f} onClick={() => setFilter(f)} className={`px-10 py-3 rounded-2xl text-[10px] font-black uppercase transition-all ${filter === f ? 'bg-white dark:bg-slate-800 shadow-3xl ' + themes[themeColor].split(' ')[0] : 'text-slate-500 hover:text-slate-200'}`}>{f}</button>
                  ))}
              </div>
      ...
      <footer className="py-20 text-center text-slate-400 text-[10px] font-black uppercase tracking-[0.8em] border-t dark:border-white/5 bg-white dark:bg-[#020617] shadow-inner">TASKPRO ECOSYSTEM • {new Date().getFullYear()} • DEVELOPED BY YESASWI</footer>
    </div>
  );
};
