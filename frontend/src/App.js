...
import { 
  ...
  Edit3, Quote, Shield, Rocket, Cpu, Code, ChevronDown, Fingerprint, Laptop, Phone, CheckCircle, Activity, Globe
} from 'lucide-react';
...
const AppContent = () => {
  ...
  const syncProfile = useCallback(async (n, t, a) => {
    ...
    } catch (err) { console.log("Profile saved locally."); }
  }, []);

  const progress = tasks.length > 0 ? (tasks.filter(t => t.status === 'Completed').length / tasks.length) * 100 : 0;
  
  // FIXED FILTER LOGIC: Grouping all non-completed tasks as "Pending"
        ...
        <div className="flex items-center gap-5">
          {!user ? (
            <div className="flex gap-4">
              <button onClick={() => {setIsRegister(false); setIsAuthOpen(true);}} className="hidden md:block text-xs font-black uppercase text-gray-500 hover:text-indigo-500 tracking-widest transition-all">Login</button>
              <button onClick={() => {setIsRegister(true); setIsAuthOpen(true);}} className="bg-indigo-600 text-white px-6 py-2.5 rounded-2xl font-black text-xs uppercase shadow-xl hover:scale-105 active:scale-95 transition-all tracking-widest">GET STARTED</button>
            </div>
          ) : (
      ...
      <AnimatePresence>
        {showProfile && user && (
          <motion.div initial={{ opacity: 0, x: 300 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 300 }} className="fixed right-0 top-0 h-full w-80 glass p-10 z-[100] border-l dark:border-zinc-800 shadow-2xl dark:bg-zinc-950 flex flex-col">
            ...
            <button onClick={() => {localStorage.clear(); window.location.href="/";}} className="w-full mt-10 p-5 rounded-3xl bg-red-500 text-white font-black uppercase text-[10px] tracking-widest shadow-xl shadow-red-500/20 hover:scale-95 transition-all"><LogOut size={16}/> Terminate Session</button>
          </motion.div>
        )}
      </AnimatePresence>
          ...
          <Route path="/" element={!user ? (
            ...
            <div className="flex flex-col items-center">
              ...
              <section className="min-h-screen py-32 px-6 max-w-7xl mx-auto flex flex-col gap-32">
                 ...
                 <div className="grid md:grid-cols-2 gap-20 items-center text-slate-900 dark:text-white">
