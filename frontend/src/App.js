...
const AppContent = () => {
  ...
  // Auth Form State (Moved here so it's in scope)
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 transition-all duration-500 font-sans">
      <nav className="glass sticky top-0 z-50 px-8 py-4 flex justify-between items-center border-b dark:border-white/5 shadow-xl">
        <div className="flex items-center gap-10">
          ...
          <div className="hidden md:flex gap-8 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">
             ...
             <Link to="/contact" className="hover:text-indigo-500 transition flex items-center gap-2 font-bold"><Mail size={14}/> Contact</Link>
          </div>
        </div>
      ...
      <AnimatePresence>
        {showProfile && user && (
          <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 50 }} className="fixed right-6 top-24 w-80 glass p-8 rounded-[3rem] z-[100] shadow-2xl border dark:border-white/10">
            ...
            <div className="mb-6 bg-slate-100 dark:bg-slate-800 p-4 rounded-2xl">
              {isEditingName ? (
                <div className="flex gap-2">
                  ...
                  <Check size={18} className="text-green-500 cursor-pointer" onClick={() => { setUser({...user, name: newName}); setIsEditingName(false); syncProfile(newName, themeColor, selectedAvatar); }} />
                </div>
              ) : (
            ...
            <button onClick={() => {localStorage.clear(); window.location.href="/";}} className="w-full p-4 rounded-2xl bg-red-500 text-white font-black flex items-center justify-center gap-2 shadow-xl shadow-red-500/20 hover:scale-95 transition-all"><LogOut size={18}/> Logout</button>
          </motion.div>
        )}
      </AnimatePresence>

      <Routes>
        <Route path="/about" element={<div className="p-20 text-center dark:text-white"><Info size={60} className="mx-auto text-indigo-500 mb-6"/><h1 className="text-5xl font-black">Internship Build</h1><p className="mt-4 text-gray-500 max-w-xl mx-auto italic">"Precision is the difference between a student and a professional."</p></div>} />
        
        <Route path="/contact" element={
          <div className="max-w-4xl mx-auto py-20 px-6 grid md:grid-cols-2 gap-12">
            <div className="dark:text-white">
              <h2 className="text-5xl font-black mb-6 tracking-tighter">Support</h2>
              <p className="flex items-center gap-3 font-bold text-gray-500"><Mail className="text-indigo-500"/> myselfadmin123@gmail.com</p>
              <p className="flex items-center gap-3 font-bold text-gray-500 mt-4"><Phone className="text-indigo-500"/> Digital Workspace Assistant</p>
            </div>
            <form className="glass p-10 rounded-[3rem] space-y-4" onSubmit={(e)=>{e.preventDefault(); alert("Sent!")}}>
              <input className="w-full p-4 glass rounded-2xl outline-none dark:text-white" placeholder="Student Name" required />
              <input className="w-full p-4 glass rounded-2xl outline-none dark:text-white" placeholder="Support Email" type="email" required />
              <textarea className="w-full p-4 glass rounded-2xl outline-none dark:text-white h-32" placeholder="Message..." required />
              <button className="w-full bg-indigo-600 text-white p-4 rounded-2xl font-black shadow-xl">Deploy Inquiry</button>
            </form>
          </div>
        } />
          ...
          <div className="min-h-[85vh] flex items-center justify-center p-6">
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="glass p-12 rounded-[3.5rem] w-full max-w-md shadow-2xl border dark:border-white/5 text-center">
              ...
              <form onSubmit={handleAuth} className="space-y-4">
                ...
                <button className={`w-full ${themes[themeColor].split(' ')[1]} text-white p-4 rounded-2xl font-black shadow-xl`}>{isRegister ? "Join" : "Sign In"}</button>
              </form>
              <button onClick={() => setIsRegister(!isRegister)} className="mt-6 text-[10px] font-black uppercase text-gray-400 underline">{isRegister ? "Already Have account" : "Create Workspace"}</button>
            </motion.div>
          </div>
        )} />
          ...
          <main className="max-w-7xl mx-auto p-6 md:p-12 grid lg:grid-cols-12 gap-12">
            <div className="lg:col-span-4 space-y-8">
              <div className={`p-10 rounded-[3.5rem] text-white shadow-2xl relative overflow-hidden ${themes[themeColor].split(' ')[1]}`}>
                ...
                <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest"><span className="flex items-center gap-2"><Trophy size={14}/> {Math.round(progress)}% Mastery</span><span className="flex items-center gap-2"><Bell size={14}/> Notifications Active</span></div>
            ...
            <div className="lg:col-span-8 space-y-8">
              <div className="flex flex-col md:flex-row gap-6 justify-between items-center bg-white dark:bg-slate-900 p-6 rounded-[2.5rem] shadow-xl border dark:border-white/5">
                ...
                <div className="flex bg-gray-100 dark:bg-slate-800 px-6 py-3 rounded-full border dark:border-white/5 gap-4 items-center">
      ...
      <footer className="py-10 text-center text-gray-500 text-[10px] font-black uppercase tracking-[0.8em] border-t dark:border-white/5">TASKPRO ECOSYSTEM • 45-DAY INTERNSHIP</footer>
    </div>
  );
};
