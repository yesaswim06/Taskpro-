...
// --- 1. ANTI-GRAVITY PARTICLE BACKGROUND ---
const ParticleBackground = () => {
  const canvasRef = useRef(null);
  useEffect(() => {
    const canvas = canvasRef.this || canvasRef.current;
...
const VirtualCursor = () => {
  ...
  useEffect(() => {
    const handleMove = (e) => setPos({ x: e.clientX, y: e.y });
  ...
  return (
    <motion.div 
      ...
      className="fixed top-0 left-0 w-6 h-6 border-2 border-indigo-500 rounded-full pointer-events-none z-[999] hidden lg:block"
    />
  );
};
...
const AppContent = () => {
  ...
  const handleAuth = async (e) => {
    ...
    try {
      ...
      setIsAuthOpen(false);
      navigate('/');
    } catch (err) { alert("Auth Failed: Connect Cloud First."); }
  };
  ...
  const syncProfile = useCallback(async (n, t, a) => {
    ...
    } catch (err) { console.log("Profile updated locally."); }
  }, []);
        ...
        <div className="flex items-center gap-5">
          {!user ? (
            <div className="flex gap-4 items-center">
              <Link to="/contact" className="text-xs font-black uppercase text-gray-500 hover:text-indigo-500 transition-all items-center gap-2"><Phone size={14}/> Support</Link>
              <button onClick={() => {setIsRegister(false); setIsAuthOpen(true);}} className="text-xs font-black uppercase text-gray-500 hover:text-white">Login</button>
          ...
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-6">
            ...
            <motion.div initial={{ scale: 0.9, y: 30 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 30 }} className="glass p-12 rounded-[3.5rem] w-full max-w-md shadow-2xl border dark:border-zinc-800 text-center dark:bg-zinc-950 relative z-[210]">
              ...
              <form onSubmit={handleAuth} className="space-y-4 text-left">
                {isRegister && <input className="w-full p-5 bg-gray-100/10 dark:bg-black rounded-3xl outline-none dark:text-white border border-white/10 focus:border-indigo-500 transition-all font-bold" placeholder="Student Name" onChange={e => setAuthForm({...authForm, name: e.target.value})} required />}
                <input className="w-full p-5 bg-gray-100/10 dark:bg-black rounded-3xl outline-none dark:text-white border border-white/10 focus:border-indigo-500 transition-all font-bold" placeholder="Email" type="email" onChange={e => setAuthForm({...authForm, email: e.target.value})} required />
                <input className="w-full p-5 bg-gray-100/10 dark:bg-black rounded-3xl outline-none dark:text-white border border-white/10 focus:border-indigo-500 transition-all font-bold" placeholder="Password" type="password" onChange={e => setAuthForm({...authForm, password: e.target.value})} required />
                <button className={`w-full ${themes[themeColor].split(' ')[1]} text-white p-5 rounded-[2rem] font-black shadow-xl mt-6 active:scale-95 transition-all tracking-widest uppercase text-xs`}>{isRegister ? "Join Now" : "Access Workspace"}</button>
              </form>
              <button onClick={() => setIsRegister(!isRegister)} className="mt-8 text-[10px] font-black uppercase text-gray-400 underline decoration-indigo-500/50 tracking-[0.2em] w-full">{isRegister ? "Login to existing" : "GET STARTED"}</button>
            </motion.div>
          </div>
        )}
      </AnimatePresence>
        ...
        {showProfile && user && (
          <motion.div initial={{ opacity: 0, x: 300 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 300 }} className="fixed right-0 top-0 h-full w-80 glass p-10 z-[100] border-l dark:border-zinc-800 shadow-2xl dark:bg-zinc-950 flex flex-col">
            ...
            <div className="space-y-10 flex-1 overflow-y-auto">
              <div className="bg-slate-100 dark:bg-zinc-900 p-4 rounded-3xl">
                {isEditingName ? (
              ...
              <div className="space-y-4">
                <button onClick={() => setShowAvatarPicker(!showAvatarPicker)} className="w-full p-2 bg-indigo-500/10 text-indigo-500 rounded-xl text-[10px] font-bold uppercase tracking-widest flex items-center justify-center gap-2">{showAvatarPicker ? "Close Gallery" : "Change Cartoon"}</button>
                {showAvatarPicker && (
                  <div className="grid grid-cols-4 gap-2 mb-6 animate-in zoom-in">
                    {avatars.map(a => <button key={a} onClick={() => {setSelectedAvatar(a); syncProfile(user.name, themeColor, a); setShowAvatarPicker(false);}} className={`p-1 rounded-xl transition-all ${selectedAvatar === a ? 'ring-2 ring-white bg-indigo-500 shadow-xl' : 'bg-slate-200 dark:bg-zinc-800'}`}><img src={`https://api.dicebear.com/7.x/pixel-art/svg?seed=${a}`} alt="a"/></button>)}
                  </div>
                )}
              </div>
              <div className="space-y-4">
        ...
        <Routes>
          <Route path="/" element={!user ? (
            <div className="flex flex-col items-center">
              ...
              {/* Scroll-Down Project Breakdown with Images */}
              <section className="min-h-screen py-32 px-6 max-w-7xl mx-auto flex flex-col gap-32">
                 <div className="grid md:grid-cols-2 gap-20 items-center">
                    ...
                    <img src="https://images.unsplash.com/photo-1550751827-4bd374c3f58b?auto=format&fit=crop&q=80&w=800" className="rounded-[4rem] shadow-4xl grayscale hover:grayscale-0 transition-all duration-700 border dark:border-white/5" alt="sec" />
                 </div>
                 <div className="grid md:grid-cols-2 gap-20 items-center">
                ...
                <div className="flex flex-col md:flex-row gap-8 justify-between items-center bg-white dark:bg-zinc-900/50 p-10 rounded-[4rem] shadow-2xl border dark:border-zinc-800">
                  ...
                  <div className="flex bg-gray-100 dark:bg-zinc-800 px-10 py-5 rounded-full border dark:border-white/5 gap-8 shadow-inner items-center">
                     <div className="flex items-center gap-4">
                        ...
                        <Timer size={28} className={isTimerRunning ? "animate-pulse text-indigo-500" : "text-gray-500"}/><span className="font-mono font-black text-3xl leading-none dark:text-white">{Math.floor(timeLeft/60)}:{timeLeft%60 < 10 ? '0' : ''}{timeLeft%60}</span>
